...
def check_macro_coverage(file_path, available_macros):
    ...
    for func in oracle_functions:
        ...
        if expected_macro and expected_macro not in available_macros:
            errors.append(
                f"Oracle function '{func.upper()}' used but no corresponding macro '{expected_macro}' found"
            )
        elif not expected_macro:
            errors.append(
                f"Oracle function '{func.upper()}' found but no macro mapping defined"
            )
...
def main():
    ...
    if all_errors:
        ...
        print("SUCCESS: Oracle macro coverage validation passed")
        if available_macros:
            print(
                f"Available Oracle utility macros: {', '.join(available_macros.keys())}"
            )
        sys.exit(0)
