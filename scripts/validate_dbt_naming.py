...
def validate_model_naming(file_path):
    ...
    if directory in naming_rules:
        ...
        if not re.match(rule["pattern"], model_name):
            errors.append(
                f"Model '{model_name}' in '{directory}' directory violates naming convention: {rule['description']}"
            )
    ...
    if model_name.startswith("_") or model_name.endswith("_"):
        errors.append(
            f"Model name '{model_name}' should not start or end with underscore"
        )

    if "__" in model_name:
        errors.append(
            f"Model name '{model_name}' should not contain double underscores"
        )
...
def validate_file_content(file_path):
    ...
    if "{{ ref(" not in content and "{{ source(" not in content:
        ...
        if "oracle_packages" not in file_path:
            errors.append(
                "Model should use {{ ref() }} or {{ source() }} for dependencies"
            )

    # Check for config block for materialization
    if "marts" in file_path and "{{ config(" not in content:
        errors.append(
            "Mart models should have {{ config() }} block for materialization"
        )
...
def main():
    ...
    if all_errors:
        ...
        print("SUCCESS: dbt model naming validation passed")
        sys.exit(0)
